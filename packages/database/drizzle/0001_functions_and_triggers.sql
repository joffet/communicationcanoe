...
CREATE OR REPLACE FUNCTION comm_canoe.resolve_identity_id(p_identity_id UUID)
RETURNS UUID
LANGUAGE plpgsql
STABLE
AS $$
...
BEGIN
  LOOP
    SELECT merged_into_id INTO next_id FROM comm_canoe.identities WHERE id = current_id;
...
CREATE OR REPLACE FUNCTION comm_canoe.identity_merge_chain_ids(p_identity_id UUID)
RETURNS SETOF UUID
LANGUAGE plpgsql
STABLE
AS $$
DECLARE
  canonical_id UUID := comm_canoe.resolve_identity_id(p_identity_id);
BEGIN
  RETURN QUERY
  WITH RECURSIVE merge_tree AS (
    SELECT canonical_id AS id
    UNION ALL
    SELECT i.id FROM comm_canoe.identities i JOIN merge_tree m ON i.merged_into_id = m.id
  )
...
CREATE OR REPLACE FUNCTION comm_canoe.resolve_conversation_id(p_conversation_id UUID)
RETURNS UUID
LANGUAGE plpgsql
STABLE
AS $$
...
BEGIN
  LOOP
    SELECT merged_into_id INTO next_id FROM comm_canoe.conversations WHERE id = current_id;
    EXIT WHEN next_id IS NULL;
    current_id := next_id;
  END LOOP;
  RETURN current_id;
...
CREATE OR REPLACE FUNCTION comm_canoe.conversation_merge_chain_ids(p_conversation_id UUID)
RETURNS SETOF UUID
LANGUAGE plpgsql
STABLE
AS $$
DECLARE
  canonical_id UUID := comm_canoe.resolve_conversation_id(p_conversation_id);
BEGIN
  RETURN QUERY
  WITH RECURSIVE merge_tree AS (
    SELECT canonical_id AS id
    UNION ALL
    SELECT c.id FROM comm_canoe.conversations c JOIN merge_tree m ON c.merged_into_id = m.id
  )
  SELECT id FROM merge_tree;
END;
$$;
...
CREATE OR REPLACE FUNCTION comm_canoe.match_document_chunks(
  p_tenant_id UUID,
...
LANGUAGE sql STABLE AS $$
  ...
  FROM comm_canoe.document_chunks
...
CREATE OR REPLACE FUNCTION comm_canoe.update_conversation_last_message_at()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  UPDATE comm_canoe.conversations
...
CREATE TRIGGER messages_update_conversation_timestamp
  AFTER INSERT ON comm_canoe.messages
  FOR EACH ROW
  EXECUTE FUNCTION comm_canoe.update_conversation_last_message_at();
...
CREATE OR REPLACE FUNCTION comm_canoe.update_conversation_response_due_at()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  IF NEW.visibility = 'external' AND NEW.direction = 'inbound' THEN
    ...
    FROM comm_canoe.tenant_settings WHERE tenant_id = NEW.tenant_id;
...
CREATE TRIGGER messages_update_conversation_response_due_at
  AFTER INSERT ON comm_canoe.messages
  FOR EACH ROW
  EXECUTE FUNCTION comm_canoe.update_conversation_response_due_at();
...
CREATE OR REPLACE FUNCTION comm_canoe.assert_chunk_tenant_matches_document()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
  IF NEW.tenant_id != (SELECT tenant_id FROM comm_canoe.documents WHERE id = NEW.document_id) THEN
...
CREATE TRIGGER document_chunks_tenant_check
  BEFORE INSERT ON comm_canoe.document_chunks
  FOR EACH ROW EXECUTE FUNCTION comm_canoe.assert_chunk_tenant_matches_document();
