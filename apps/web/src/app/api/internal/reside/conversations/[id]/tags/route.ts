...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { tenantId, tagName } = parsed.data;
  ...
  const existing = await domain.listTenantTags(tenantId);
  let tag = existing.find((t) => t.name.toLowerCase() === tagName.trim().toLowerCase());
  if (!tag) {
    try {
      tag = await domain.createTag(tenantId, tagName.trim());
    } catch (err) {
      ...
      const retried = await domain.listTenantTags(tenantId);
